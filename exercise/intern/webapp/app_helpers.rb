...
module AppHelpers
  ...
  def underactivity_penalty(_user)
    0
  end
  ...
  def offer_maturation_date(offer)
    ...
    base_url = 'https://www.timeanddate.com/worldclock/fixedtime.html'
    "<a target='_blank' style='color: #{color}' href='#{base_url}?iso=#{date_iso}&p1=217'>#{date}</a>"
  end
  ...
  def offer_awardee(offer)
    ...
    qry  = "amendment_uuid = '#{offer.position.amendment.uuid}' AND side = '#{offer.position.contract.awardee}'"
    uuid = Position.where(qry).first&.user&.uuid
    user_type = uuid == offer.user&.uuid ? 'trader' : 'worker'
  ...
  def escrow_awardee(escrow)
    ...
    query = "amendment_uuid = '#{escrow.amendment.uuid}' AND side = '#{escrow.contract.awardee}'"
    user  = Position.where(query).first.user
    user_name(user)
  end
